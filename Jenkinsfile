                ...
                milestone(1)
                 withCredentials([sshUserPrivateKey(credentialsId: 'webserver_login', keyFileVariable: 'key', passphraseVariable: 'USERNAME', usernameVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker run --restart always --name train-schedule -p 8080:8080 -d shaaba/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
